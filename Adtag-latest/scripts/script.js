...
function showRecordsForAdmin(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    table +=
    `<tr>
      <td style="text-align: center;">${arr[i]["date"].split("-").reverse().join("-")}</td>
      ...
      <td style="text-align: center;">
        <input id="${arr[i]["id"]}" class="stat-input" type="text" value="${arr[i]["status"]}" />
